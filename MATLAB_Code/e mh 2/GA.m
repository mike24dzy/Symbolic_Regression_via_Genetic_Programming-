...
for i = 1:times
    i
    for j = 1:group/2
        rand = randperm(group,2);
        r1 = rand(1);
        r2 = rand(2);
        r = randi([1,63]);
        [p1 p2] = crossover(gene(r1,1:63),gene(r2,1:63),r);
        r = randi([1,63]);
        c1 = mutation(p1,r);
        gene(group+2*j-1,1:63) = c1;
        y1 = get_plot(1,c1,x0);
        gene(group+2*j-1,64) = get_fit(y1,y0);
        r = randi([1,63]);
        c2 = mutation(p2,r);
        y2 = get_plot(1,c2,x0);
        gene(group+2*j,1:63) = c2;
        gene(group+2*j,64) = get_fit(y2,y0);
    end
    datal(:,i) = gene(:,64);
...
save('GA1.mat','datal','len0','yf','genef','str')
toc
